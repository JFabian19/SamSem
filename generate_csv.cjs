...
let categoriasCsv = 'nombre,orden\n';
let platosCsv = 'categoria,nombre,descripcion,precio,orden\n';
...
MENU_DATA.menu.forEach((cat, index) => {
    ...
    cat.items.forEach((item, itemIndex) => {
        const ordenItem = itemIndex + 1;
        const desc = item.descripcion ? item.descripcion.replace(/"/g, '""') : '';
        platosCsv += `"${cat.categoria}","${item.nombre}","${desc}",${item.precio},${ordenItem}\n`;
    });
});

fs.writeFileSync('categorias.csv', categoriasCsv);
