...
// Create a directory for CSVs to keep them organized
const outputDir = path.join(__dirname, 'csv_export');
if (!fs.existsSync(outputDir)) {
    fs.mkdirSync(outputDir);
}

// Generate Categories CSV (Master List)
let categoriasCsv = 'nombre,orden,gid\n'; // added gid column placeholder
MENU_DATA.menu.forEach((cat, index) => {
    const ordenCat = index + 1;
    categoriasCsv += `"${cat.categoria}",${ordenCat},\n`;
});
fs.writeFileSync(path.join(outputDir, '00_CATEGORIAS_MASTER.csv'), categoriasCsv);
console.log('Master Categories CSV created.');

// Generate Individual Category CSVs
MENU_DATA.menu.forEach((cat) => {
    let content = 'nombre,descripcion,precio,orden\n';

    cat.items.forEach((item, index) => {
        const orden = index + 1;
        const desc = item.descripcion ? item.descripcion.replace(/"/g, '""') : '';
        content += `"${item.nombre}","${desc}",${item.precio},${orden}\n`;
    });

    // Sanitize filename
    const filename = cat.categoria.replace(/[^a-z0-9]/gi, '_').toLowerCase() + '.csv';
    fs.writeFileSync(path.join(outputDir, filename), content);
    console.log(`Created ${filename}`);
});

console.log('All CSVs generated in /csv_export folder.');
